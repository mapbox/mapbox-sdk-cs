...
namespace Mapbox.UnitTest
{
    ...
    internal class TileCoverTest
    {
        ...
        public void World()
        {
            ...
            {
                var tiles = TileCover.Get(GeoCoordinateBounds.World(), zoom);
        ...
        public void Helsinki()
        {
            ...
            var sw = new GeoCoordinate(60.163200, 24.937700);
            var ne = new GeoCoordinate(60.163300, 24.937800);

            var set1 = TileCover.Get(new GeoCoordinateBounds(sw, ne), 13);
            Assert.AreEqual(1, set1.Count);

            var list1 = new List<CanonicalTileId>(set1);
            Assert.AreEqual("13/4663/2371", list1[0].ToString());

            var set2 = TileCover.Get(new GeoCoordinateBounds(sw, ne), 6);
            Assert.AreEqual(1, set2.Count);

            var list2 = new List<CanonicalTileId>(set2);
            Assert.AreEqual("6/36/18", list2[0].ToString());

            var set3 = TileCover.Get(new GeoCoordinateBounds(sw, ne), 0);
